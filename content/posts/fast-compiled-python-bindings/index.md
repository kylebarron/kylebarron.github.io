...
These areas are independent, and striving to tackle all three is what makes a library lightning-fast. As this post is focused on Python integration, we'll focus just on the first two.
...
The tricky bit is that we need to have an idea of how the end Python user will be using our library so that we can adapt the Python API to what they need to perform. Since end users will likely be writing code in pure Python, the more functionality end users must implement themselves, the worse their overall performance will be.
...
2. Another that uses Shapely's vectorized [`shapely.intersects`], where you pass in an array of geometries and the single Python dispatch will operate on _all of them at once_.
...
Any time a native function is called from Python, the native code needs to be able to access and operate on the Python input. For simple parameters like integers and strings, this is straightforward and something usually handled by the binding glue generated by [pyo3](https://github.com/PyO3/pyo3), [Cython](https://github.com/cython/cython), [pybind](https://github.com/pybind/pybind11), etc. But for more complex input like structured tabular data, you usually have to choose your own serialization method.
...
One way around this is to call [`shapely.to_geojson`](https://shapely.readthedocs.io/en/stable/reference/shapely.to_geojson.html) on the input. That returns a [GeoJSON](https://geojson.org/) string that you can then parse with a JSON library in your compiled code.

```py
%timeit shapely.to_geojson(multi_polygon)
# 2.9 ms ± 73.1 μs per loop (mean ± std. dev. of 7 runs, 100 loops each)

len(shapely.to_geojson(multi_polygon))
# 345735 (bytes)
```
...
Shapely supports a compact binary serialization format for geometries called ["Well-Known Binary"](https://libgeos.org/specifications/wkb/) (WKB). So instead of going through GeoJSON, we can call [`shapely.to_wkb`](https://shapely.readthedocs.io/en/stable/reference/shapely.to_wkb.html) on the input, creating a binary buffer that we can parse from the compiled side with a WKB parser.

```py
%timeit shapely.to_wkb(multi_polygon)
# 184 μs ± 5.78 μs per loop (mean ± std. dev. of 7 runs, 10,000 loops each)

len(shapely.to_wkb(multi_polygon))
# 143917 (bytes)
```

This is 15x as fast and the serialization is more 2.4x as compact as a JSON string, but this still requires two full memory copies. One copy from the Shapely source into a WKB binary buffer, and then a second copy still for the deserialization into the target memory format.
